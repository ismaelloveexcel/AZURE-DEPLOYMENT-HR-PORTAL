...
export function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    "dashboard" | "employees" | "compliance" | "recruitment" | "settings"
  >("dashboard");
  ...
  const fetchComplianceAlerts = useCallback(async () => {
    ...
    try {
      const res = await fetchWithAuth(
        `${API_BASE}/employees/compliance/alerts?days=60`,
        {
          token: user.token,
          role: user.role,
        },
  ...
  const toggleFeature = async (key: string, enabled: boolean) => {
    ...
    try {
      const res = await fetchWithAuth(
        `${API_BASE}/admin/features/${key}?is_enabled=${enabled}`,
        {
          method: "PUT",
          token: user.token,
          role: user.role,
      ...
      if (res.ok) {
        setFeatures(
          features.map((f) =>
            f.key === key ? { ...f, is_enabled: enabled } : f,
          ),
        );
      }
    } catch (err) {
  ...
  const complianceBadge = complianceAlerts
  ...
  return (
    <>
      ...
      <AppShell
        ...
        subtitle="Operations, compliance, and staffing insights for Baynunah HR. Use the navigation to switch between workstreams."
      >
        <div className="space-y-10">
          <div className="flex flex-wrap items-center gap-2 bg-white/70 border border-primary-200 rounded-full p-2 shadow-sm">
            <TabButton
              ...
              <section className="surface-section">
                <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                  <div>
                    ...
                    <p className="text-sm text-primary-500">
                      Jump directly into key admin flows.
                    </p>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <QuickActionCard
                    ...
                    description="Directory, profiles, and master data"
                    onClick={() => setActiveTab("employees")}
                  />
                  <QuickActionCard
                    icon={<ComplianceIcon />}
                    title="Compliance alerts"
                    description="Visas, Emirates ID, ILOE, and contracts"
                    onClick={() => setActiveTab("compliance")}
                  />
                  <QuickActionCard
                    icon={<RecruitmentIcon />}
                    title="Recruitment pipeline"
                    description="Requests, candidates, and interviews"
                ...
                <div className="space-y-3">
                  {features.map((feature) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() =>
                          toggleFeature(feature.key, !feature.is_enabled)
                        }
...
function TabButton({
  ...
  return (
    ...
    >
      <span className="w-4 h-4 text-accent-green flex items-center justify-center">
        {icon}
      </span>
      <span>{label}</span>
      {badge !== undefined && badge > 0 && (
        <span className="chip chip--danger text-xs leading-none">{badge}</span>
      )}
    </button>
  );
}
...
function StatCard({
  ...
  return (
    <div className="stat-card">
      ...
      <span
        className={`stat-value ${colorMap[color as keyof typeof colorMap]}`}
      >
        {value}
      </span>
    </div>
...
function QuickActionCard({
  ...
  return (
    <button
      onClick={onClick}
      type="button"
      className="text-left p-5 border border-primary-100 rounded-2xl bg-white/90 hover:border-accent-green/30 hover:shadow-card transition-all"
    >
      <div className="w-10 h-10 text-accent-green mb-3 flex items-center justify-center bg-accent-green/10 rounded-xl">
        {icon}
      </div>
      <p className="font-medium text-primary-900 mb-1">{title}</p>
