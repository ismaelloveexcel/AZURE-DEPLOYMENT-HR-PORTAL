...
- Either `AZURE_WEBAPP_PUBLISH_PROFILE_BACKEND` (publish profile for `hrportal-backend-new`) **or**
  OIDC credentials as repository secrets (`AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`) with Contributor or Web Plan Contributor permissions on the backend resource group for the backend deploy workflow
- `AZURE_STATIC_WEB_APPS_TOKEN` (deployment token for `hrportal-frontend-new`)
- Repository variable `AZURE_BACKEND_RESOURCE_GROUP` set to your backend resource group (default from azd is `rg-hr-portal-new`) when using the OIDC path
