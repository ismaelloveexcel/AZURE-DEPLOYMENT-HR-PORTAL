...
- Either `AZURE_WEBAPP_PUBLISH_PROFILE_BACKEND` (publish profile for `hrportal-backend-new`) **or**
  OIDC credentials as repository secrets (`AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`) with permissions on the backend resource group for the backend deploy workflow
